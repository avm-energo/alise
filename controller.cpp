...
void Controller::shutdown() {
  QMetaObject::invokeMethod(m_runner, &ZeroRunner::stopServer,
                            Qt::DirectConnection);
}
...
Controller *Controller::ofType(Controller::ContrTypes type) {
  ...
  connect(m_pingTimer, &QTimer::timeout, m_runner,
          &ZeroRunner::publishHealthQueryCallback);
  m_pingTimer->start();
  return this;
}

void Controller::adminjaSetup() {
  connect(m_runner, &ZeroRunner::timeReceived, m_timeSynchronizer,
          &TimeSyncronizer::printAndSetSystemTime);
#if defined(AVTUK_STM)
  connect(m_runner, &ZeroRunner::timeReceived, m_deviceBroker,
          &Broker::setTime);
  connect(m_runner, &ZeroRunner::timeRequest, m_deviceBroker, &Broker::getTime);
#else
  connect(
      m_runner, &ZeroRunner::timeRequest, m_timeSynchronizer, //
      [this] { m_deviceBroker->updateTime(m_timeSynchronizer->systemTime()); });
#endif
  connect(m_deviceBroker, &Broker::receivedTime, m_runner,
          &ZeroRunner::publishTime, Qt::DirectConnection);
  connect(m_timeSynchronizer, &TimeSyncronizer::ntpStatusChanged, this,
          [&](bool status) {
...
void Controller::booterSetup() {
  connect(m_runner, &ZeroRunner::healthReceived, m_deviceBroker,
          &Broker::healthReceived);
}

void Controller::coreSetup() {
  connect(m_deviceBroker, &Broker::receivedBlock, m_runner,
          &ZeroRunner::publishBlock);
}

bool Controller::isIncorrectType() { return (m_type == IS_INCORRECT_TYPE); }
