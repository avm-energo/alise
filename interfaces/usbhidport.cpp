...
UsbHidPort::UsbHidPort(const UsbHidSettings &dev, QObject *parent)
    : BasePort("UsbHidPort", parent), m_deviceInfo(dev), m_hidDevice(nullptr),
      m_waitForReply(false), m_shouldBeStopped(false) {
...
QByteArray UsbHidPort::read(bool *status) {
  ...
  if ((bytes == 0) && (m_waitForReply) &&
      (missingCounter == missingCounterMax)) {
    constexpr int responseSize = 5;
    unsigned char timeoutResponse[responseSize] = {0x3c, 0xf0, 0x01, 0x00,
                                                   0x01};
...
bool UsbHidPort::shouldBeStopped() const { return m_shouldBeStopped; }
...
void UsbHidPort::usbEvent(const USBMessage message, quint32 type) {
  ...
  case DBT_DEVICEARRIVAL: {
    ...
    if (match.captured(0) != "USB" && match.captured(0) != "HID")
      break;
    if (m_deviceInfo.hasMatch(message.guid)) {
      if (!shouldBeStopped())
        deviceConnected();
      shouldBeStopped(false);
      break;
    }
    if (m_deviceInfo.hasPartialMatch(message.guid)) {
      if (!shouldBeStopped())
        deviceConnected();
      shouldBeStopped(false);
      break;
...
bool UsbHidPort::writeDataToPort(QByteArray &ba) {
  ...
  int errorCode = hid_write(
      m_hidDevice, reinterpret_cast<unsigned char *>(ba.data()), tmpt); // write
  if (errorCode == -1)
    return false;
