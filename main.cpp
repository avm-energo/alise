...
int main(int argc, char *argv[]) {
  ...
  QSettings settings("/root/sonica/alise/settings/settings.ini",
                     QSettings::IniFormat);
  int logcounter = settings.value("Test/counter", "1").toInt();
  settings.setValue("Test/counter", ++logcounter);
  QString logFileName =
      settings.value("Logs/logfile", "/root/sonica/alise/logs/alise.log")
          .toString();
  ...
  AliseConstants::setFailureBlinkPeriod(
      settings.value("Timers/FailureBlink", "50").toInt());
  AliseConstants::setSonicaStartingBlinkPeriod(
      settings.value("Timers/StartingBlink", "250").toInt());
  AliseConstants::setSonicaNormalBlinkPeriod(
      settings.value("Timers/NormalBlink", "500").toInt());
  AliseConstants::setPowerCheckPeriod(
      settings.value("Timers/PowerCheckPeriod", "1000").toInt());
  AliseConstants::setResetCheckPeriod(
      settings.value("Timers/ResetCheckPeriod", "1000").toInt());
  AliseConstants::setHealthQueryPeriod(
      settings.value("Timers/HealthQueryPeriod", "4000").toInt());
  AliseConstants::setGpioBlinkPeriod(
      settings.value("Timers/GpioBlinkPeriod", "50").toInt());
...
void listPins() {
  for (auto &cit : ::gpiod::make_chip_iter()) {
    ...
    for (auto &lit : ::gpiod::line_iter(cit)) {
      ...
      std::cout.width(8);
      std::cout << (lit.direction() == ::gpiod::line::DIRECTION_INPUT
                        ? "input"
                        : "output");
      std::cout << " ";
