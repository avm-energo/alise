...
ZeroPublisher::ZeroPublisher(zmq::context_t &ctx, int sock_type,
                             QObject *parent)
...
template <typename T>
void ZeroPublisher::appendToQueue(const std::string &id, const T &paylod) {
...
void ZeroPublisher::publishPowerStatus(const AVTUK_CCU::Main powerStatus) {
  qDebug() << "PowerStatus has been added to output queue: "
           << powerStatus.PWRIN << ", resetReq: " << powerStatus.resetReq;
...
void ZeroPublisher::publishHello(const QString id, const quint32 code) {
  qDebug() << "HelloReply has been added to output queue: " << id
           << ", code: " << code;
...
void ZeroPublisher::send(itemType &str) {
  ...
  qInfo() << "Send message to: {" << str.first.c_str() << "}, with payload: {"
          << str.second.c_str() << "}";
